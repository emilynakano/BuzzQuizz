...
function pegarseusquizzes() {
    ...
    const dadosSerializados = JSON.stringify(arraycomSeusQuizzes);
    localStorage.setItem("lista", dadosSerializados);
    const listaSerializada = localStorage.getItem("lista");
    const listaquizzcriados = JSON.parse(listaSerializada);
    console.log(`esta é a lista ${listaquizzcriados}`)
